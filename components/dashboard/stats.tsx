...
const DashboardStats = ({ netWorth, monthIncome, monthExpense, cashFlow }: DashboardStatsProps) => {
  ...
  return (
    ...
    const DashboardStats = ({ netWorth, monthIncome, monthExpense, cashFlow, goalValue, projectionValue, progressPercent, status }: DashboardStatsProps) => {
      const router = useRouter()
      // Status visual
      const statusMap = {
        ok: { label: "No ritmo", color: "bg-green-500", icon: <TrendingUp className="h-4 w-4 text-green-500" /> },
        risco: { label: "Risco", color: "bg-yellow-500", icon: <TrendingFlat className="h-4 w-4 text-yellow-500" /> },
        atrasado: { label: "Atrasado", color: "bg-red-500", icon: <TrendingDown className="h-4 w-4 text-red-500" /> },
      }
          ...
          <Card className="col-span-2 lg:col-span-2 ring-2 ring-primary/30 shadow-lg">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium flex items-center gap-2">
                Central Financeira
                <span title="Visão integrada: valor atual, meta, projeção e status" className="cursor-help text-muted-foreground">&#9432;</span>
              </CardTitle>
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-xl font-bold">{goalValue !== undefined ? formatCurrency(goalValue) : "-"}</div>
                  </div>
                  <div>
                    <div className="text-xs text-muted-foreground">Projeção</div>
                    <div className="text-xl font-bold">{projectionValue !== undefined ? formatCurrency(projectionValue) : "-"}</div>
                  </div>
                  <div className="flex flex-col items-center">
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${statStatus.color} text-white flex items-center gap-1`}>
      ...
      >
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">Receitas do mês – Despesas do mês</p>
        </CardContent>
      </Card>
    </div>
  )
